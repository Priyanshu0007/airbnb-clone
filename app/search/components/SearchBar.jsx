...
const SearchBar = ({toggleExpanded}) => {
  ...
  const handleSelect = (ranges) => {
    ...
    setDateRangeLabel(
      `${ranges.selection.startDate.getDate()}/${ranges.selection.startDate.getMonth()+1} - ${ranges.selection.endDate.getDate()}/${ranges.selection.endDate.getMonth()+1}`
    );
  };
  ...
  return (
    <div className="flex flex-row self-center justify-center rounded-full border p-2 mt-8 w-3/4">
      <button className="border-r px-4 text-left grow" onClick={() => setIsSearchFocused(true)}>
        <p className="font-bold">Where</p>
        {isSearchFocused ? (
          <input type="text" placeholder="Search destinations" onChange={handleLocationUpdate} value={locationInput} className="text-slate-800 bg-transparent border-none outline-none" />
        ) : (
          <p className="text-slate-600">
            {locationInput && locationInput !== "" ? locationInput : "Search Destinations"}
          </p>
        )}
      </button>
      <div className="dropdown dropdown-end px-4 border-r">
        ...
        <div tabIndex={1} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52" >
          <DateRangePicker ranges={[selectionRange]} minDate={new Date()} onChange={handleSelect} rangeColors={["#FF385C"]} />
        </div>
      </div>
      ...
      <button onClick={handleSearchClick} className="px-4 text-white rounded-full bg-primary p-4 flex justify-center gap-3 flex-row" >
